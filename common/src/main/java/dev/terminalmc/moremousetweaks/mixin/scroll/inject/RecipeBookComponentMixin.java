...
public abstract class RecipeBookComponentMixin implements IRecipeBookWidget {
    ...
    public ScrollAction mmt$scrollRecipeBook(double mouseX, double mouseY, double scrollAmount) {
        ...
        if (options().scrollRecipeBookPages && mouseX >= left && mouseX < left + 147) {
            // Page scrolling
            // Ugly approach since assigning the cast value causes a runtime mixin error
            ...
            ((IRecipeBookResults) recipeBookPage).mmt$setCurrentPage(Mth.clamp(
                    (int) (((IRecipeBookResults) recipeBookPage).mmt$getCurrentPage() + Math.round(
                            scrollAmount)), 0, Math.max(maxPage, 0)
            ));
            ((IRecipeBookResults) recipeBookPage).mmt$refreshResultButtons();
            return ScrollAction.SUCCESS;
        } else if (options().scrollRecipeBookTabs && mouseX >= left - 30 && mouseX < left) {
            // Tab scrolling
